...
function bindVisualiserEvents(){ const canvas=$('#constellationCanvas'); if(!canvas||canvas.dataset.bound)return; canvas.dataset.bound='1'; const point=e=>{const r=canvas.getBoundingClientRect(); const t=e.touches?.[0]||e; return {x:t.clientX-r.left,y:t.clientY-r.top};}; const down=e=>{const p=point(e); const hit=[...(viz.hits||[])].reverse().find(h=>Math.hypot(h.x-p.x,h.y-p.y)<=h.r); if(hit){inspectVisualNode(hit.node); return;} viz.drag={x:p.x,y:p.y,rot:viz.rotation,tilt:viz.tilt,panX:viz.panX,panY:viz.panY,pan:e.shiftKey||viz.interaction==='pan'}; canvas.setPointerCapture?.(e.pointerId);}; const move=e=>{ if(!viz.drag)return; const p=point(e), dx=p.x-viz.drag.x, dy=p.y-viz.drag.y; if(viz.drag.pan){viz.panX=viz.drag.panX+dx;viz.panY=viz.drag.panY+dy;} else {viz.rotation=viz.drag.rot+dx*.006;viz.tilt=Math.max(.15,Math.min(1.35,viz.drag.tilt+dy*.004));} }; const up=()=>{viz.drag=null;}; canvas.addEventListener('pointerdown',down); canvas.addEventListener('pointermove',move); canvas.addEventListener('pointerup',up); canvas.addEventListener('pointercancel',up); canvas.addEventListener('wheel',e=>{e.preventDefault(); viz.zoom=Math.max(.45,Math.min(4,viz.zoom*(e.deltaY>0?.92:1.09)));},{passive:false}); }
...
async function loadMemories(){
  ...
  $('#memoryList').innerHTML=data.items.map(memoryItem).join('') || '<div class="empty">No memories.</div>';
...
function memoryItem(m){ return `<div class="memory-item" data-rid="${esc(m.rid)}"><div class="memory-title">${esc(m.domain||'general')} <span class="muted">· ${esc(m.type)}</span></div><div class="memory-text">${esc(m.text)}</div><div class="meta-row"><span class="pill hot">${esc(m.consolidation_status)}</span><span class="pill">imp ${Number(m.importance||0).toFixed(2)}</span><span class="pill">${esc(m.source||'source')}</span><span class="pill">${esc(m.created_at_iso||'')}</span></div></div>`; }
